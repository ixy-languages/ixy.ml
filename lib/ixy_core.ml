...
let get_reg hw register =
  get_reg_fast hw (IXGBE.register_to_reg register)

let set_reg hw register data =
  set_reg_fast hw (IXGBE.register_to_reg register) data
...
module Make (Pci : Ixy_pci.S) = struct
  let wait_clear hw register mask =
    while (get_reg hw register) land mask <> 0 do () done

  let wait_set hw register mask =
    while (get_reg hw register) land mask <> mask do () done

  module Memory = Ixy_pci.Make (Pci)
  ...
  module RXD = struct
    ...
    let reset cs Ixy_memory.{ phys; _ } =
  ...
  module TXD = struct
    ...
    let reset cs Ixy_memory.{ size; phys; _ } =
  ...
  let init_rx ra pci n =
    ...
    if n > 0 then begin
      ...
      let init_rxq i =
        ...
        let descriptor_ring =
          match Memory.allocate_dma pci ~require_contiguous:true ring_size_bytes with
          | None -> error "could not allocate descriptor_ring"
          | Some mem -> mem in
        let open Ixy_memory in
        ...
        (* set base address *)
        let lo, hi = split descriptor_ring.physical in
        ra.set_reg (IXGBE.RDBAL i) lo;
        ra.set_reg (IXGBE.RDBAH i) hi;
        (* set ring length *)
        ra.set_reg (IXGBE.RDLEN i) ring_size_bytes;
        debug "rx ring %d phy addr: %#018Lx" i descriptor_ring.physical;
  ...
  let init_tx ra pci n =
    if n > 0 then begin
      ...
      let init_txq i =
        ...
        let descriptor_ring =
          match Memory.allocate_dma pci ~require_contiguous:true ring_size_bytes with
          | None -> error "could not allocate descriptor ring"
          | Some mem -> mem in
        Cstruct.memset descriptor_ring.virt 0xff;
        (* set base address *)
        let lo, hi = split descriptor_ring.physical in
        ...
        debug "tx ring %d phy addr: %#018Lx" i descriptor_ring.physical;
        let txdctl_old =
        ...
        let txds =
          TXD.split
            ...
            descriptor_ring.virt in
        let tx_bufs =
          Array.make num_tx_queue_entries dummy in
  ...
  let wait_for_link t =
    let rec loop () =
      ...
      | _, false
      | `SPEED_UNKNOWN, _ -> loop ()
      | `SPEED_10G, true -> info "Link speed is 10 Gbit/s"
      | `SPEED_1G, true -> info "Link speed is 1 Gbit/s"
      | `SPEED_100, true -> info "Link speed is 100 Mbit/s" in
    loop ()
  ...
  let create ~pci ~rxq ~txq =
    ...
    let Ixy_pci.{ vendor; device_id; class_code; subclass; prog_if } =
      Pci.get_config pci in
    let pci_addr = Pci.to_string pci in
    begin match class_code, subclass, prog_if, vendor with
      | 0x2, 0x0, 0x0, v when v = Ixy_pci.vendor_intel -> ()
      | 0x1, 0x0, 0x0, v when v = Ixy_pci.vendor_intel ->
        error
          "device %s is configured as SCSI storage device in EEPROM"
          pci_addr
      | 0x2, 0x0, _, v when v <> Ixy_pci.vendor_intel ->
        error
    ...
    let hw =
      Pci.map_resource pci in
    let ra =
      { get_reg = get_reg hw;
        set_reg = set_reg hw;
        set_flags = set_flags hw;
        clear_flags = clear_flags hw;
        wait_set = wait_set hw;
        wait_clear = wait_clear hw
      } in
    ...
    info "reset done";
    info "initializing device %s" pci_addr;
    ...
    let t =
      { pci;
        ...
        pci_addr;
        num_rxq = rxq;
        rxqs = init_rx ra pci rxq;
        num_txq = txq;
        txqs = init_tx ra pci txq;
        ra;
  ...
  let rx_batch ?(batch_size = max_int) t rxq_id =
    ...
    let bufs =
      ...
      let receive offset =
        ...
        buf.Ixy_memory.size <- (RXD.size [@inlined]) rxd;
    ...
    if num_done > 0 then begin
      ...
      set_reg_fast t.hw rdt (wrap_rx (rxq.rx_index - 1))
    end;
    bufs
  ...
  let tx_batch t txq_id bufs =
    ...
    set_reg_fast t.hw tdt txq.tx_index;
