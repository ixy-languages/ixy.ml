...
Both ixy and ixy.ml only support rings whose descriptor capacity is a power of two: With this ring size, wrapping around the ring's end can be done without costly modulo operations (`next_index <- (current_index + 1) & (ring_size - 1)`, see `wrap_rx` and `wrap_tx` in [`lib/ixy_core.ml`](../lib/ixy_core.ml)).
